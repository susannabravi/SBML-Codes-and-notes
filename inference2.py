...
def load_model(base_model_path, dora_path):
    """Load base model + DoRA adapters."""
    ...
    # Load DoRA adapters
    if dora_path:
        # First check the PEFT config to confirm it's DoRA
        peft_config = PeftConfig.from_pretrained(dora_path)
        print(f"PEFT config type: {peft_config.peft_type}")
        print(f"Using DoRA: {peft_config.use_dora}")
        print(f"Task type: {peft_config.task_type}")
        
        # Load the PEFT model with DoRA adapters
        model = PeftModel.from_pretrained(model, dora_path)
        print("DoRA adapters loaded successfully!")
        
        # For DoRA, we generally don't merge and unload immediately
        # as DoRA has a more complex structure with magnitude vectors
        # If you want to merge for faster inference, uncomment the next line:
...
def generate_response(model, tokenizer, instruction, max_new_tokens=256):
    ...
    with torch.no_grad():
        try:
            # Try with DoRA-compatible generation parameters
            outputs = model.generate(
                **inputs,
                ...
                # DoRA might be more sensitive to generation parameters
                top_k=50,  # Add top_k filtering
                early_stopping=True
            )
        except Exception as e:
            print(f"Generation failed with sampling, trying greedy: {e}")
...
def main():
    parser = argparse.ArgumentParser(description="Test fine-tuned model with DoRA adapters")
    parser.add_argument("--base_model", default="deepseek-ai/deepseek-coder-v2-lite-base", 
                       help="Base model path")
    parser.add_argument("--dora_path", default="./output_test", 
                       help="DoRA adapters path")
    ...
    parser.add_argument("--merge_adapters", action="store_true", 
                       help="Merge DoRA adapters into base model for faster inference")
    
    args = parser.parse_args()
    
    # Load model
    model, tokenizer = load_model(args.base_model, args.dora_path)

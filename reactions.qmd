...
print(f"So there are {first_len - len(df2)} rows where all the columns are the same, also the meta_id of the snippet.")
```
...
print(f"Rows: {len(different_notes)}")
...
print(f"Rows: {len(multiple_reactions_per_note)}")
...
conflicting_notes_rows.sort_values(by='notes').head(15)
```
...
```{python}
unique_per_value = df2.groupby("reaction_id")["notes"].nunique()
sorted_counts = unique_per_value.sort_values()
